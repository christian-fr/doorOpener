...
from app.api.util import get_state, set_state, log, add_user, add_scope, add_valid, check_if_admin_by_api_key, \
...
def set_door_state():
    if request.args.get('api-key') is None:
    ...
    elif request.args.get('actor') is None:
        return Response(
        ...
        if user_id is None:
            ...
            try:
                set_state(uuid.UUID(request.args.get('actor')), user_id)
...
def get_door_state():
    if request.args.get('api-key') is None:
        ...
        if user_id is None:
            ...
            try:
                result = get_state(uuid.UUID(request.args.get('actor')), user_id)
                if result is None:
                    return response_permission_error()
                else:
